...
impl Bar {
    pub fn pop_piece(&mut self, player_number: i8) -> Result<Piece, &'static str> {
        let index = self.pieces.iter().position(|p| p.player_number == player_number); 
        match index {
...
mod tests {
    ...
    fn pop_piece_valid_test() {
        let player_one_piece = Piece { player_number: 1 };
        let player_two_piece = Piece { player_number: 2 };
